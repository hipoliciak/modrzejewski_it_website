...
**Trigger Scenario:** A customer record is processed in Data Cloud. We need to evaluate their incoming preference and establish the baseline consent state for their target contact point email.
...
2. **Decision (`Check Consent Status`):** Evaluate the incoming registration payload data to determine the user's intent.
* *Branch A (Opt In):* Triggering data indicates active subscription acceptance.
* *Branch B (Opt Out):* Triggering data indicates explicit non-consent or omission.
3. **Action Branches:**
...
* **Trigger Condition:** A record is created or updated. Object anchor set to `Consent Giver ID (Contact)`.
* **Logic Steps:**
...
| **3. CRM --> MC Next** | Automation Event-Triggered | `CommSubscriptionConsent` (Created/Updated) | Native `Create Consent` (Action) |
